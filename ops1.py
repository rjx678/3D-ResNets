...
def bias_variable(name, shape):
    ...
    return tf.get_variable('b_' + name, dtype=tf.float32,
                           initializer=initial)
#weight=3*7*7 strides=1*2*2
def new_conv_layer1(inputs,  # The previous layer.
                   layer_name,
                   stride,
    ...
    shape = [3, filter_size, filter_size, num_inChannel, num_filters]
    ...
    return layer
#weight=3*3*3 1*1*1 strides=1*1*1 2*2*2
...
def max_pool(x, ksize, stride, name):
    ...
    return tf.nn.max_pool3d(x,
                            ...
                            name=name)
#stride=1*2*2
def max_pool1(x, ksize, stride, name):
    """Create an max pooling layer."""
    return tf.nn.max_pool3d(x,
                            ksize=[1, ksize, ksize, ksize, 1],
                            strides=[1, 1, stride, stride, 1],
                            padding="SAME",
...
def avg_pool(x, ksize, stride, name):
    ...
    return tf.nn.avg_pool3d(x,
                            ...
                            padding="VALID",
                            name=name)
#ksize=1*7*7 stride=1*1*1
def avg_pool1(x, ksize, stride, name):
    """Create an average pooling layer."""
    return tf.nn.avg_pool3d(x,
                            ksize=[1, 1, ksize, ksize, 1],
                            strides=[1, stride, stride, stride, 1],
                            padding="VALID",
                            name=name)
...
#used in 18&34 layers
def bottleneck_block1(x, num_ch, block_name,
                     s1, k1, nf1, name1,
                     s2, k2, nf2, name2,
                     s3, k3, name3, first_block=False):
    # num_ch = x.get_shape()[-1]
    with tf.variable_scope(block_name):
        # Convolutional Layer 1
        layer_conv1 = new_conv_layer(inputs=x,
        ...
        if first_block:
            shortcut = new_conv_layer(inputs=x,
                                      ...
                                      use_relu=False)
            assert (
                shortcut.get_shape().as_list() == layer_conv2.get_shape().as_list()), "Tensor sizes of the two branches are not matched!"
            res = shortcut + layer_conv2
        else:
            res = layer_conv2 + x
            assert (
                x.get_shape().as_list() == layer_conv2.get_shape().as_list()), "Tensor sizes of the two branches are not matched!"
    return tf.nn.relu(res)
